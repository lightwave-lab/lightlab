    ...
    def virtual(self):
        ...
        if self._virtual is None:
            raise VirtualizationError(
                "Virtual context unknown." "Please refer to method asVirtual()."
            )
        else:
...
class DualInstrument(Virtualizable):
    ...
    def __init__(self, real_obj=None, virt_obj=None):
        ...
        if real_obj is not None and virt_obj is not None:
            violated = []
            allowed = (
                real_obj.essentialMethods
                + real_obj.essentialProperties
                + dir(VirtualInstrument)
            )
            ...
            if len(violated) > 0:
                logger.warning(
                    "Virtual instrument ({}) violates ".format(type(virt_obj).__name__)
    ...
    def virtual(self, toVirtual):
        ...
        if toVirtual and self.virt_obj is None:
            raise VirtualizationError(
                "No virtual object specified in", type(self.real_obj)
            )
        elif not toVirtual and self.real_obj is None:
            raise VirtualizationError(
                "No real object specified in", type(self.virt_obj)
            )
    ...
    def __getattribute__(self, att):
        ...
        """
        if att in (
            list(DualInstrument.__dict__.keys()) + list(Virtualizable.__dict__.keys())
        ):
