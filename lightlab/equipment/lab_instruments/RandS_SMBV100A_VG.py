...
class RandS_SMBV100A_VG(VISAInstrumentDriver, Configurable):
    ...
    def carrierMod(self, enaState=True, typMod=None, deviation=None, modFreq=None):
        ...
        if typMod is None:
            ...
            if enaState:
                raise Exception(
                    "Modulation type must be selected with typMod= to enable"
                )
        ...
        if typMod not in allMods:
            raise Exception(
                typMod + " is not a valid kind of carrier modulation: am, fm, or pm"
            )
    ...
    def listEnable(
        self, enaState=True, freqs=None, amps=None, isSlave=False, dwell=None
    ):
        """ Sets up list mode.
