...
class ILX_7900B_LS(VISAInstrumentDriver, MultiModuleConfigurable):
    ...
    def __init__(self, name="The laser source", address=None, useChans=None, **kwargs):
        ...
        MultiModuleConfigurable.__init__(
            self, useChans=useChans, configModule_klass=ILX_Module
        )
    ...
    def setConfigArray(self, cStr, newValArr, forceHardware=False):
        ...
        """
        wroteToHardware = super().setConfigArray(
    ...
    def enableState(self, newState):
        ...
        for ena in newState:
            if ena not in [0, 1]:
                raise ValueError(
                    "Laser states can only be 0 or 1. " + "Got {}".format(newState)
                )
    ...
    def wls(self, newWls):
        ...
        for iCh, wl in enumerate(newWls):
            ...
            if wl < wlRanges[0]:
                logger.warning(
                    "Wavelength out of range was constrained:\n"
    ...
    def powers(self, newPowers):
        ...
        for iCh, level in enumerate(newPowers):
            if level < self.powerRange[0]:
                logger.warning(
                    "Power out of range was constrained:\n"
