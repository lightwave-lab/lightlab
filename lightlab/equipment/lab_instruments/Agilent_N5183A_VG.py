...
class Agilent_N5183A_VG(VISAInstrumentDriver, Configurable):
    ...
    def amplitude(self, amp=None):
        ...
        if amp is not None:
            if amp > 15:
                print(
                    "Warning: Agilent N5183 ony goes up to +15dBm, given {}dBm.".format(
                        amp
                    )
                )
                amp = 15
            if amp < -20:
                print(
                    "Warning: Agilent N5183 ony goes down to -20dBm, given {}dBm.".format(
                        amp
                    )
                )
                amp = -20
    ...
    def frequency(self, freq=None):
        ...
        if freq is not None:
            if freq > 40e9:
                logger.warning(
                    "Agilent N5183 ony goes up to 40GHz, given %s GHz.", freq / 1e9
                )
                freq = 40e9
            if self.sweepEnable():
                logger.warning(
