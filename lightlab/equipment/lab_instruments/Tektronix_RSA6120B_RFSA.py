...
class Tektronix_RSA6120B_RFSA(VISAInstrumentDriver, Configurable):
    ...
    def sgramTransfer(self, duration=1.0, nLines=100):
        ...
        if downsample < 1:
            ...
            print(
                "Warning: line density too high. You will get {} lines.".format(nLines)
            )
            downsample = 1
    ...
    def spectrum(self, freqReso=None, freqRange=None, typAvg="none", nAvg=None):
        ...
        freqRangeActual[0] = float(
            self.getConfigParam("SPEC:FREQ:START", forceHardware=True)
        )
        freqRangeActual[1] = float(
            self.getConfigParam("SPEC:FREQ:STOP", forceHardware=True)
        )
