...
class ILX_7900B_LS(VISAInstrumentDriver):
    ...
    TODO: This class WILL have a special property in that lockouts occur on a channel basis
    ...
    safeSleepOn = {}
    safeSleepOn['enable'] = 3
    safeSleepOn['wavelength'] = 30
    safeSleepOn['level'] = 5

    powerRange = np.array([-20, 13])

    def __init__(self, name='The laser source', address=None, **kwargs):
    ...
    def startup(self):
        ...
        if stateDict is None:
            ...
            self.stateDict = dict([ch, -1] for ch in self.channels)
        else:
            self.channels = list(stateDict.keys())
            self.stateDict = stateDict
        # if any(ch > self.fullChannelNums - 1 for ch in self.channels):
        #     raise Exception('Requested channel is more than there are available')
        if not set(self.ordering_left).isdisjoint(self.channels):
            self.ordering = self.ordering_left
        elif not set(self.ordering_left).isdisjoint(self.channels):
            self.ordering = self.ordering_right
        self.sleepOn = self.safeSleepOn
    ...
    def enableState(self, newState):
        ...
        self.stateDict = dict(zip(self.channels, enforcedState))
    ...
    def moduleIterate(self, attrStr, virtualSetVals=None):
        ...
        for iModule in range(len(self.ordering)):  # iterate over modules
            ...
            if orderedChan in self.channels:  # only enter for modules that have been reserved
                virtualChan = self.channels.index(orderedChan)
    ...
    def off(self):
        ...
        """
        self.enableState = np.zeros(len(self.channels))
