...
from lightlab.util import io, data
...
class Tektronix_DSA8300_Oscope(VISAInstrumentDriver, Configurable):
    ...
    '''
    # def __init__(self, isDPO=False, hostname='andromeda'):
    def __init__(self, name='The fast oscilloscope', address=None, **kwargs):
        VISAInstrumentDriver.__init__(self, name=name, address=address, **kwargs)
        self.dpo = False
        # s = 'visa://' + hostNS[hostname] + '/GPIB' + str(0) + '::' + str(14) + '::INSTR'
        # super().__init__('The fast oscilloscope', s, tempSess=True)
        self.totalChans = 8
    ...
    def startup(self):
        ...
        recLenParam = ':HORIZONTAL' + ':MAIN:RECORDLENGTH'#+ (':RECORDLENGTH')
    ...
    def __transferData(self, chan):
        ...
        forcing = True
        chStr = 'CH' + str(chan)
        self.setConfigParam(':DATA:ENCDG', 'ASCII', forcing)
        self.setConfigParam(':DATA:SOURCE', chStr, forcing)
        ...
        for p in wfmInfoParams:
            wfmInfo[p] = float(self.getConfigParam('WFMOUTPRE:' + p.upper(), forcing))
        if self.dpo:
        ...
        timeScale = float(self.getConfigParam(':HORIZONTAL:MAIN:SCALE', forcing))
