...
class Agilent_N5222A_NA(VISAInstrumentDriver, Configurable):
    ...
    def frequency(self, freq=None):
        ...
        if freq is not None:
            if freq > 26e9:
                print(
                    "Warning: Agilent N5183 ony goes up to 40GHz, given {}GHz.".format(
                        freq / 1e9
                    )
                )
                freq = 26e9
            ...
            if self.sweepEnable():
                print(
                    "Warning: Agilent N5183 was sweeping when you set frequency, moving to CW mode"
                )
    ...
    def sweepEnable(self, swpState=None):
        ...
        if swpState is not None:
            ...
            if self.swpRange is not None:
                self.setConfigParam(
                    "SENS:FREQ:STAR", self.swpRange[0], forceHardware=True
                )  # Hack
                self.setConfigParam(
                    "SENS:FREQ:STOP", self.swpRange[1], forceHardware=True
                )  # Hack
    ...
    def measurementSetup(self, measType="S21", chanNum=None):
        ...
        if (
            len(activeMeasTypes) == 1
            and measType == activeMeasTypes[0]
            and newMeasName == activeMeasNames[0]
        ):
            ...
            self.setConfigParam(
                "CALC{}:PAR:EXT".format(chanNum),
                "'{}', '{}'".format(newMeasName, measType),
                forceHardware=True,
            )
            self.setConfigParam(
                "DISP:WIND:TRACE{}:FEED".format(traceNum),
                "'{}'".format(newMeasName),
                forceHardware=True,
            )
            changed = True
        self.setConfigParam(
            "CALC{}:PAR:MNUM".format(self.chanNum), self.chanNum, forceHardware=changed
        )
