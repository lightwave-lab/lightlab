...
class Anritsu_MP1763B_PPG(VISAInstrumentDriver, Configurable):
    ...
    def syncSource(self, src=None):
        ...
        if src is not None:
            try:
                ...
                raise ValueError(src + " is not a valid sync source: " + str(tokens))
    ...
    def bitseq(self, chpulses, clockfreq, ext=0, addplot=False, mult=1, res=5):
        ...
        for delay in chpulses:
            for pulses in chpulses[delay]:
                ...
                pIndex = int(
                    np.round((max(chpulses.keys()) - delay + pulsePos) * clockfreq)
                )
        ...
        if addplot:
            T = np.linspace(
                0, mult * len(pattern) / clockfreq, mult * res * len(pattern)
            )
    ...
    def PRBS_pattern(cls, order, mark_ratio=0.5):
        ...
        prbs_pattern = list(
            islice(iter(prbs_generator(polynomial, seed)), 2 ** order - 1)
        )
