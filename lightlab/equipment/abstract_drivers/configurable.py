...
class TekConfig(object):
    ...
    def getList(self, subgroup="", asCmd=True):
        ...
        children = dpath.util.search(
            self.dico, subgroup + "*", yielded=True, separator=self.separator
        )
        for cmd in children:
            s, v = cmd
    ...
    def __parseShorthand(cls, setResponse):
        ...
        for i in range(len(pairs)):
            ...
            if len(words) > 2:
                print(
                    "Warning 2-value returns not handled by TekConfig class. Ignoring..."
                )
                print(*words)
...
class Configurable(AbstractDriver):
    ...
    def __init__(
        self,
        headerIsOptional=True,
        verboseIsOptional=False,
        precedingColon=True,
        interveningSpace=True,
        **kwargs
    ):
    ...
    def saveConfig(self, dest="+user", subgroup="", overwrite=False):
        ...
        if type(dest) in [TekConfig, dict]:
            ...
            self.config[dest[1:]].transfer(self.config["live"], subgroup=subgroup)
    ...
    def loadConfig(self, source="+user", subgroup=""):
        ...
        if type(source) in [TekConfig, dict]:
            ...
            if (
                source[1:] == "default" and self.config["default"] is None
            ):  # need to load default
                self.config["default"] = TekConfig.fromFile(self.getDefaultFilename())
    ...
    def generateDefaults(self, filename=None, overwrite=False):
        ...
        if Path(filename).exists() and not overwrite:
            logger.warning(
                "%s already exists." "Use `overwrite` if you really want.", filename
            )
            return
