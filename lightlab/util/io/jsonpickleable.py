...
class HardwareReference(object):
    ...
    def open(self):
        raise TypeError(
            f"This object is placeholder for a real "
            f"{self.klassname}. "
...
class JSONpickleable(Hashable):
    ...
    def __getstate__(self):
        ...
        for key, val in state.copy().items():
            if isinstance(key, str):
                ...
                elif val.__class__.__name__ == "VISAObject" or any(
                    base.__name__ == "VISAObject" for base in val.__class__.mro()
                ):
                ...
                # 3. functions that are not available in modules - saves the code text
                    ...
                    val
                ) and not jsonpickle.util.is_module_function(val):
    ...
    def _fromJSONcheck(cls, json_string):
        ...
        context = jsonpickle.unpickler.Unpickler(
            backend=jsonpickle.json, safe=True, keys=True
        )
        try:
