...
class NdSweeper(Sweeper):
    ...
    def __init__(self):
        ...
        self.plotOptions = {
            ...
            "yKey": None,
            "axArr": None,
            "cmap-surf": matplotlib.cm.inferno,
            "cmap-curves": matplotlib.cm.viridis,
    ...
    def gather(
        self, soakTime=None, autoSave=False, returnToStart=False
        ...
        try:
            ...
            for index in np.ndindex(self.swpShape):
                pointData = (
                    OrderedDict()
                )  # Everything that will be measured *at this index*
                ...
                for iDim, actu in enumerate(self.actuate.items()):
                    ...
                    if actuObj.domain is None:
                        ...
                        pointData[actuKey] = x
                    if (
                        iDim == self.actuDims - 1
                        or index[iDim + 1] == 0
                        or actuObj.doOnEveryPoint
                    ):
    ...
    def plot(self, slicer=None, tempData=None, index=None, axArr=None, pltKwargs=None):
        ...
        if len(yKeys) == 0:
            ...
            for datKey, datVal in fullData.items():
                if (
                    datKey not in xKeys
                    and datKey not in actuationKeys
        ...
        if len(xKeys) == 0 or len(yKeys) == 0:
            raise ValueError(
                "No axis key specified explicitly or found in self.actuate"
            )
        for k in xKeys + yKeys:
            if k not in fullData.keys():
                raise KeyError(
                    k
                    + " not found in data keys. "
                    + "Available data are "
                    + ", ".join(fullData.keys())
                )
        ...
        if axArr is not None:
            ...
            _, axArr = plt.subplots(
                nrows=plotArrShape[0],
                ncols=plotArrShape[1],
                figsize=(10, plotArrShape[0] * 2.5),
        ...
        for iAx, ax in np.ndenumerate(axArr):
            ...
            if self.plotOptions["plType"] == "curves":
                if plotDims == 1:
                    ...
                    # legend
                    if (
                        autoLabeling and iAx[0] == 0 and iAx[1] == plotArrShape[1] - 1
                    ):  # AND it is the top right
                        ax.legend(bbox_to_anchor=(1.05, 1), loc=2, borderaxespad=0.0)
                else:
                    raise ValueError(
                        "Too many dimensions in sweep to plot. "
    ...
    def __repr__(self):
        ...
        for data_key, data_val in self.data.items():
            data_str.append(
                '<{} {}> "{}"'.format(data_val.shape, data_val.dtype, data_key)
            )
...
class CommandControlSweeper(Sweeper):
    ...
    def __init__(self, evaluate, defaultArg, swpInds, domain, nTrials=1):
        ...
        if len(self.domain) != self.swpDims:
            raise ValueError(
                "domain and swpInds must have the same dimension."
    ...
    def gather(
        self, autoSave=False, randomize=False
    ):  # pylint: disable=arguments-differ
        """ Executes the sweep
        ...
        prog = io.ProgressWriter(
            swpName, (self.nTrials,) + self.swpShape, **self.monitorOptions
        )
    ...
    def toSweepData(self):
        ...
        """
        monitorInds = tuple(
            filter(lambda y: y not in self.swpInds, range(self.allDims))
        )
        cmdMat = self.cmdGrid
    ...
    def plot(self, index=None, axArr=None):
        ...
        if plType == "cmdErr":
            ...
            plType == "curves" and self.swpDims == 1
            ...
            (
                ...
                measMat,
                monitMat,
            ) = self.toSweepData()  # pylint: disable=unused-variable
            xFull = cmdMat[:, 0]
    ...
    def score(self, bits=False, worstCase=False):
        ...
        consolidateErrorVsWeight = (
            lambda x: np.max(np.abs(x)) if worstCase else rms(x, axis=None)
        )
...
def plotCmdCtrl(sweepData, index=None, ax=None, interactive=False):
    ...
    if index is None:
        ...
        if is2D:
            ...
            plotCmdCtrl(
                sweepData, index=(measWeights.shape[0], 0, 0), interactive=False
            )
    else:
        ...
        if not is2D:
            ...
            if index[0] > 0:
                ...
                elli = plotCovEllipse(
                    cov, mean, volume=0.5, ax=interAx, ec="b", fc="none"
                )
            else:
                elli = None
...
def availablePlots(dims=None, swpType=None):
    ...
    for k, v in pTypes.items():
        if dims is None or dims in v[0]:
            if (
                swpType is None
                or type(swpType) is type
                and swpType.__name__ in v[1]
                or type(swpType) is str
                and swpType in v[1]
            ):
                avail.append(k)
    return avail
...
def assertValidPlotType(plType, dims=None, swpClass=None):
    if plType not in availablePlots(dims, swpClass):
        ...
        errStr.append(
            "Available plots are: {}".format(", ".join(availablePlots(dims, swpClass)))
        )
        logger.error("\n".join(errStr))
