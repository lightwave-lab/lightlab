...
def config_save(config, omit_default=True):
    ...
    if not os.access(user_config_path, os.W_OK):
        print(
            f"Write permission to {user_config_path} denied. You cannot save. Try again with sudo."
        )
        return False
    with open(user_config_path, "w") as user_config_file:
        ...
        print(
            "----{}----".format("-" * len(f"saving {user_config_path}")),
            file=sys.stderr,
        )
    return True
...
config_cmd_parser.add_argument(
    ...
    nargs="?",
    choices=("write-default", "get", "set", "reset"),
    metavar="command",
)
...
def config_main(args):
    ...
    if config_args.action == "write-default":
        ...
        if len(params) == 2:
            ...
            raise SystemExit(
                f"Invalid syntax. Use lightlab config set section.item value."
            )
        ...
        if len(params) == 1:
            ...
            raise SystemExit(
                f"Invalid syntax. Use lightlab config reset section[.item]"
            )
    else:
