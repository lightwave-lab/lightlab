...
class FunctionBundle(Hashable):  # pylint: disable=eq-without-hash
    ...
    def addDim(self, newMeasFun):
        if self.absc is None:
            ...
            y = self._putInTimebase(
                newMeasFun
            )  # This does the checking for type and timebase
    ...
    def __getitem__(self, index):
        ...
        if type(index) is int:
            theOrdi = self.ordiMat[
                index, :
            ].A1  # A1 is a special numpy thing that converts from matrix to 1-d array
    ...
    def __eq__(self, other):
        return (
            self.memberType is other.memberType
            and self.absc == other.absc
            and self.ordiMat == other.ordiMat
        )
    ...
    def __getattr__(self, attrName):
        ...
        try:
            ...
            betterErrStr = err.args[0] + ", neither does '{}' object".format(
                type(self).__name__
            )
    ...
    def multiAxisPlot(self, *args, axList=None, titleRoot=None, **kwargs):
        ...
        if len(axList) != len(self):
            raise ValueError(
                "Wrong number of axes. Got {}, need {}.".format(len(axList), len(self))
            )
    ...
    def componentAnalysis(
        self, *args, pcaIca=True, lNorm=2, expectedComponents=None, **kwargs
    ):
