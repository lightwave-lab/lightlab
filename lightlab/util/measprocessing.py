...
class SpectrumMeasurementAssistant(object):
    ...
    def __init__(self, nChan=1, arePeaks=False, osaRef=None):
        ...
        if osaRef is None:
            print(
                "Warning: osa is not initialized as of laboratory-state update. It will be phony"
            )
        self.osa = osaRef
        self.__backgrounds = {}
    ...
    def rawSpect(self, avgCnt=1):
        if self.osa is None:
            raise Exception(
                "This is a phony instance with no spectrum analyzer present"
            )
    ...
    def resonances(self, spect=None, avgCnt=1):
        ...
        if self.filtShapesForConvolution is not None:
            (
                fineRes,
                confidence,
            ) = spect.refineResonanceWavelengths(  # pylint: disable=unused-variable
    ...
    def killResonances(self, spect=None, avgCnt=1, fwhmsAround=3.0):
        """
        """
        if spect is None:
            spect = self.fgSpect(avgCnt=avgCnt)
    ...
    def fgResPlot(
        self, spect=None, axis=None, avgCnt=1
    ...
    def getBgSpect(self, bgType=None):
        ...
        if bgType is None:
            ...
            try:
                ...
                raise KeyError(
                    "Background of type '" + bgType + "' has not been taken yet."
                )
        else:
            raise ValueError(
